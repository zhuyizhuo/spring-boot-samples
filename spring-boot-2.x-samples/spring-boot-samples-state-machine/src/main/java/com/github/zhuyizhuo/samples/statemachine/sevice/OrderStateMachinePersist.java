...
@Slf4j
@Component
public class OrderStateMachinePersist implements StateMachinePersist<OrderStatusEnum, OrderStatusChangeEvent, Order> {
    
    @Override
    public void write(StateMachineContext<OrderStatusEnum, OrderStatusChangeEvent> stateMachineContext, Order order) throws Exception {
    ...
    public StateMachineContext<OrderStatusEnum, OrderStatusChangeEvent> read(Order order) throws Exception {
        ...
        return StringUtils.hasText(order.getStatus()) ?
                new DefaultStateMachineContext<>(OrderStatusEnum.valueOf(order.getStatus()), null, null, null, null, StateMachineConstant.ORDER_STATE_MACHINE_ID) :
                new DefaultStateMachineContext<>(OrderStatusEnum.CREATED, null, null, null, null, StateMachineConstant.ORDER_STATE_MACHINE_ID);
    }
}
